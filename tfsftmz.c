...
static int firstX = 0,  // indices for first point in TF region /*@\label{tfsftmzA}@*/
           lastX;       // indices for last point in TF region  /*@\label{tfsftmzB}@*/
...
*/
  ...
  Dz1G(g1, 0) += ezInc(TimeG(g1), 0.0); // set source node
  ...
  for (nn = 0; nn <SizeY ; nn++)
    Dz(mm, nn) -= Cezh(mm, nn) * Hy1G(g1, mm - 1);
